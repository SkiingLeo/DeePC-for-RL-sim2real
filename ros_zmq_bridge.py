...
ZMQ_INBOUND_TOPICS = [
    ...
    "/digital_twin/torque_differences",  # JointState
]
...
def franka_state_to_payload(msg: FrankaRobotState) -> Dict[str, Any]:
    js_meas = msg.measured_joint_state
    js_des = msg.desired_joint_state
    return {
        # positions/velocities stay measured; torques use desired_joint_state.effort
        "position": list(js_meas.position[:NUM_JOINTS]),
        "velocity": list(js_meas.velocity[:NUM_JOINTS]) if js_meas.velocity else None,
        "effort": list(js_des.effort[:NUM_JOINTS]) if js_des.effort else None,
    }
...
class RosZmqBridge(Node):
    ...
    def __init__(self):
        ...
        self._torque_diff_pub = self.create_publisher(JointState, "/digital_twin/torque_differences", 10)
    ...
    def _recv_loop(self) -> None:
        while self._running.is_set():
            ...
            elif topic == "/digital_twin/torque_differences":
                msg = payload_to_joint_state(payload, stamp)
                msg.name = [f"torque_diff_{i+1}" for i in range(NUM_JOINTS)]
                self._torque_diff_pub.publish(msg)
            else:
